...
const FlippedSlides = ({ attributes }) => {
    ...
    const CYCLE_PAUSE_MS = 1500; // Pause between UP and DOWN, and between DOWN and UP (loop)
    ...
    // NEW: Ref to manage the continuous loop timer
...
 
    ...
    // -----------------------------------------------------------------
    // CORRECTED: Infinite Staggered Up/Down Loop Logic
    // -----------------------------------------------------------------
    useEffect(() => {
        if (isMobile || scrollData.length === 0) return;
        ...
        const runSequence = (direction, onComplete) => {
            ...
            const nextStepTimer = setTimeout(() => {
                // Ensure transitions are cleared before the next move/loop starts
                columnRefs.current.forEach(ref => {
                    if (ref.current) {
                        ref.current.style.transition = 'none';
                    }
        ...
        return () => {
            ...
            // This cleanup logic needs to catch all running timers
            // We rely on the browser/React unmounting to handle the timeouts in the runSequence closures
            // But explicitly resetting the column positions is crucial.
            columnRefs.current.forEach(ref => {
                if (ref.current) {
                    ref.current.style.transition = 'none';
                    ref.current.style.transform = 'translateY(0px)';
