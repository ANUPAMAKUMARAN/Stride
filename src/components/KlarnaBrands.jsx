...
const KlarnaBrands = ({ attributes }) => {
    ...
    const styles = {
        ...
        slideItemBase: {
            display: 'block', // Added to make it a block element for link
            borderRadius: scaleValue(20),
            overflow: 'hidden',
            position: 'relative',
            backgroundSize: 'cover',
            ...
            flexShrink: 0,
            transition: 'transform 0.3s ease-out',
            textDecoration: 'none', // Ensure link wrapper doesn't underline content
            color: 'inherit', // Ensure text color is inherited
        },
        // Inner content for the link wrapper
        slideContentInner: {
            position: 'relative',
            width: '100%',
            height: '100%',
    ...
    const renderCardContent = (slide, cardId, isMobileView) => {
        const isHovered = hoveredIndex === cardId;
        const linkUrl = slide.linkUrl || '#'; // Use '#' as a fallback if no linkUrl is provided
        ...
        const cardStyle = {
            ...
            transform: isHovered ? 'scale(1.03)' : 'scale(1)',
            // Change cursor back to default if no link is provided, though 'pointer' is in slideItemBase
        ...
        // Wrap the card content in an anchor tag if linkUrl is present
        if (hasLink) {
            return (
                <a 
                    key={cardId}
                    ...
                    style={{ textDecoration: 'none', display: 'block' }} // Style for the outer link wrapper
                >
                    {CardInnerContent}
                </a>
            );
        ...
        // Return just the div if no link is present
        return (
            <div key={cardId}>
                {CardInnerContent}
            </div>
    ...
    return (
        <div style={styles.container}>
            <header style={styles.header}>
                <h2 style={styles.title}>{title}</h2>
                {/* The "See all" link remains an empty anchor tag with a placeholder style */}
                <a href="#" style={styles.seeAll}>
                    See all
                </a>
            </header>
